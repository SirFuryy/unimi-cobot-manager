...
# Leggere i file di allarme del controller e del servo
...
class DobotApi:
    def __init__(self, ip, port, gui, *args):
        """
    inizializza e stabilisce la connessione con il dobot attraverso le socket, crean il threading lock e inizializza il file di log.
	Parametri: riferimento, l'indirizzo ip (192.168.5.1 nel nostro setup), la porta (29999, 30003 o 30004) e l'interfaccia grafica su cui stampare (GUI). Args finisce dentro il file di log."""
        self.ip = ip
        self.port = port
        self.gui = gui
        self.socket_dobot = 0
        ...
        if self.port == 29999 or self.port == 30003 or self.port == 30004 or self.port == 30005:
            try:
                ...
                raise Exception(
                    f"Unable to set socket connection while using port {self.port} !", socket.error)
        else:
            raise Exception(
                f"Connect to dashboard server need use port 29999, 30003 o 30004 !")

    def log_command(self, text):
        """
    Stampa sul terminale "Invio_comandi" il testo passato come parametro con il timestamp e lo salva sul relativo file di log.
	Parametri: il riferimento e il testo del log
        """
        date = datetime.datetime.now().strftime("%H:%M:%S ")
        if self.text_log:
            self.text_log.insert(END, date + text + "\n")
        self.gui.write_to_terminal(3, date + text + "\n")
	...
	Parametri: il riferimento e il testo del log
        """
        date = datetime.datetime.now().strftime("%H:%M:%S ")
        self.gui.write_to_terminal(4, date + text + "\n")

    def send_data(self, string):
        """
    invia la stringa al dobot come una socket e lo salva come log.
	Parametri: il riferimento e la stringa da inviare.
        """
        self.log_command(f" -- send to {self.port} - {string}")
        try:
    ...
    def wait_reply(self):
        """
    richiede via socket una risposta al dobot, la restituisce e la invia ai log.
	Parametri: riferimento
	Returns: la risposta ricevuta
    """
        data = ""
        try:
            ...
            if len(data) == 0:
                data_str = "no data recived"
            else:
                data_str = str(data, encoding="utf-8")
            self.log_command(f' -- receive from {self.port} - {data_str}')
            return data_str

    def sendRecvMsg(self, string):
        """
    wrappa e unisce le funzioni send_data e wait_reply rendendole sincronizzate, ovvero richiede un lock del thread verso il dobot (ovvero solo lui può eseguire queste chiamate finchè è lockato), esegue un send data con il testo preso come parametro e aspetta lòa risposta, poi la ritorna.
	Parametri: riferimento e istruzioni da passare al robot
	Returns: risposta del robot
    """
    ...
    def close(self):
        """
    chiude la connessione socket con il robot
	Parametri: riferimento
    """
    ...
    def __del__(self):
        """
    wrappa ed esegue la funzione close.
	Parametri: riferimento
    """
        self.close()
...
class DobotApiDashboard(DobotApi):
  ...
  """
    def EnableRobot(self, load=0.0, centerX=0.0, centerY=0.0, centerZ=0.0):
      """
    Invia il comando di enable al robot con i valori che gli vengono passati in input e ritorna la risposta del robot.
	Parametri: riferimento, peso handler, (x, y, z) del centro del referance frame che vogliamo utilizzare (di base è 0, 0, 0 ed è il centro del flange).
	Returns: risposta del robot
    """
      string = 'EnableRobot('
      if load != 0:
    ...
    def DisableRobot(self):
        """
    Invia al robot il comando di disable e ritorna la risposta del robot
	Parametri: riferimento
	Returns: la risposta del robot
    """
        string = "DisableRobot()"
        return self.sendRecvMsg(string)
    ...
    def ClearError(self):
        """
    Invia al robot il segnale di pulisci errori e ritorna la risposta.
	Parametri: riferimento
	Returns: la risposta del robot
    """
        string = "ClearError()"
    ...
    def ResetRobot(self):
        """
    Invia la robot un segnale di stop immediato e ritorna la risposta del robot.
	Parametri: riferimento
	Returns: la risposta del robot
    """
        string = "ResetRobot()"
    ...
    def SpeedFactor(self, speed):
        """
    Invia al robot l'istruzione di modificare la velocità globale di movimento in un valore tra lo 0 e il 100% e ritorna la risposta del robot
	Parametri: riferimento, il valore di velocità che si vuole impostare (intero tra 1 e 100 compresi)
	Returns: la risposta del robot
    """
    ...
    *dynParams :Parameter Settings（Mode、Distance、Index、Status）
    ...
    """
        ...
        string = "MovJIO({:f},{:f},{:f},{:f},{:f},{:f}".format(
            x, y, z, a, b, c)
        self.log_command("Send to 192.168.5.1:29999:" + string)
        for params in dynParams:
            string = string + "," + str(params)
        string = string + ")"
        return self.sendRecvMsg(string)
...
class DobotApiFeedBack(DobotApi):
    def __init__(self, ip, port, gui, *args):
        super().__init__(ip, port, gui, *args)
        self.__MyType = []
    ...
    def log_feedback(self, message):
        date = datetime.datetime.now().strftime("%H:%M:%S ")
        self.gui.write_to_terminal(5, date + message + "\n")
    
    def log_error(self, message):
        date = datetime.datetime.now().strftime("%H:%M:%S ")
        self.gui.write_to_terminal(4, date + message + "\n")

    def feedBackData(self):
        """ Return the robot status every 200ms """
        self.socket_dobot.setblocking(True)  # 设置为阻塞模式
        self.socket_dobot.settimeout(1)  # impostato per l feedback da 200ms
        data = bytes()
        temp = bytes()
        ...
        try:
            ...
            self.log_feedback("Socket timeout at point 1 occurred while receiving data, sleeping")
            try:
                self.socket_dobot.close()
                self.socket_dobot = socket.socket()
                self.socket_dobot.connect((self.ip, self.port))
                self.socket_dobot.setblocking(True)  # 设置为阻塞模式
                self.socket_dobot.settimeout(1)  # 设置超时时间为0.1秒
            except socket.error:
                self.log_error(socket.error)
                raise Exception(
                    f"Unable to set socket connection while using port {self.port} !", socket.error)
        ...
        if len(temp) > 1440:    
            try:
                ...
                self.log_feedback("Socket timeout for exceeded 1440 bytes occurred while receiving data, sleeping")
                raise Exception("Socket timeout occurred while receiving data for exceeded 1440 bytes")
        ...
        if len(temp) < 1440:
            while i < 5:
                try:
                    ...
                    self.log_feedback("Socket timeout occurred while receiving data in third branch, try n: " + str(i))
                    try:
                        self.socket_dobot.close()
                        self.socket_dobot = socket.socket()
                        self.socket_dobot.connect((self.ip, self.port))
